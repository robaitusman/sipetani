...
class PermissionsController extends Controller
{
	...
	function index(Request $request, $fieldname = null , $fieldvalue = null){
		...
		$query->leftJoin("roles", "permissions.role_id", "=", "roles.role_id");
     ...
     */
	function add(){
		// Ambil semua route yang relevan untuk permission
		$allRoutes = \Route::getRoutes();
		$routeOptions = [];
		foreach ($allRoutes as $route) {
			$uri = $route->uri();
			// Filter: hanya route web, exclude api, komponen, fileuploader, info, dsb
			if (
				!preg_match('/^(api|componentsdata|fileuploader|info)/', $uri)
				&& strpos($uri, '{') === false // exclude route dengan parameter
				&& $route->methods()[0] !== 'HEAD' // exclude HEAD
			) {
				$routeOptions[] = $uri;
			}
		}
		sort($routeOptions);
		return $this->renderView("pages.permissions.add", compact('routeOptions'));
	}
