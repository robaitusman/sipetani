...
$(document).ready(function(){
    ...
    try {
        ...
        const targetPaths = ['/indikatormaster/input', '/masteradata/input'];
        if (targetPaths.includes(currentPath)) {
            // Hapus class active dari semua nav-link
            $('.nav-link').removeClass('active');
            // Tambahkan class active ke nav-link yang menuju ke evaluasi kinerja
            const targetLink = $('a[href*="periodeevaluasi/evaluasi"]');
            if (targetLink.length > 0) {
                targetLink.addClass('active');
                // Optional: Buka parent dropdown jika ada
                targetLink.closest('.dropdown').find('.dropdown-toggle').addClass('active');
                targetLink.closest('.collapse').addClass('show');
            }
        }
    } catch (error) {
        console.error('Error setting active navigation:', error);
    }
    $('#ctrl-alamat').addressPickerByGiro({
                showMap: true,
                mapOptions: {
                    zoom: 15,
                    zoomNoValue: 15,
