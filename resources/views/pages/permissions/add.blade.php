...
<?php
    $pageTitle = "Add New Permissions"; //set dynamic page title
?>
@extends($layout)
                                    ...
                                    <div class="row">
                                        ...
                                        <div class="col-sm-8">
                                            <div id="ctrl-permission-holder" class=" ">
                                                <select id="ctrl-permission" data-field="permission" name="permission" required class="form-select">
                                                    <option value="">Pilih Permission (Route)</option>
                                                    @foreach($routeOptions as $route)
                                                        <option value="{{ $route }}" {{ get_value('permission') == $route ? 'selected' : '' }}>{{ $route }}</option>
                                                    @endforeach
                                                </select>
                                            </div>
                                        </div>
...
$(document).ready(function(){
    $('#ctrl-permission').select2({
        placeholder: "Pilih Permission (Route)",
        allowClear: true,
        width: '100%'
    });
});
